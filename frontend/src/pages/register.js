  ...
  return (
    <section className="px-4 py-16 md:px-8">
      <div className="mx-auto max-w-2xl rounded-[2rem] border border-slate-700/60 bg-slate-950/80 p-10 shadow-glass">
        <h1 className="text-3xl font-semibold text-white">Register for CamEvent Explorer</h1>
        <p className="mt-3 text-slate-400">Create your account to save events and book tickets.</p>
        <form onSubmit={handleSubmit} className="mt-10 space-y-6">
          <label className="block text-sm text-slate-300">
            Full name
            <input value={name} onChange={(e) => setName(e.target.value)} required className="mt-2 w-full rounded-3xl border border-slate-700 bg-slate-900 px-4 py-4 text-white outline-none focus:border-cambodia-400" />
          </label>
          <label className="block text-sm text-slate-300">
            Email
          ...
          <button className="w-full rounded-full bg-cambodia-500 px-6 py-4 text-sm font-semibold text-white hover:bg-cambodia-400">Create account</button>
        </form>
